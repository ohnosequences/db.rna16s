...
case object rna16sIdentification {
  ...
  */
  lazy val is16s: Entry => Dropped + Entry =
    database.filterIncluded(_)
      .right.flatMap(annotation.hasRNAType16s)
      .right.flatMap(annotation.someDescriptionContains16s)
      .right.flatMap(sequence.has16sLength)
      .right.flatMap(sequence.qualityOK)
    ...
    entry => {
      ...
      val includedEntryAnnotations =
        entry.entryAnnotations
          .filter { a => includedDatabases contains a.databaseEntry.database }
  ...
  case object sequence {
    ...
    entry =>
      if(hasLength16s(entry))
        ...
        Left(Dropped.InvalidSequenceLength(entry.rnaSequence.sequence.length, entry))
    ...
    */
    lazy val qualityOK: Entry => Dropped + Entry =
      entry =>
        if(
          ratioOfNs(entry.rnaSequence.sequence) <= 0.01       &&
